...
public class Simulator {
	...
	* 
	...
	private static void mutationEvent() {
		...
		double mut2 = event.time() + RandomUtils.getRandomTime((1 - Math.log10(population.fitness(event.individual())) * MUT_INTERVAL / 10.0));
		Event eventMutation = new Event('m', event.time() +  RandomUtils.getRandomTime((1 - Math.log10(population.fitness(event.individual())) * MUT_INTERVAL)), event.individual());
	...
	private static void reproductionEvent() {
		...
		double fit = 1 - Math.log10(population.fitness(event.individual()));
		
		//double mut = RandomUtils.getRandomTime(fit * MUT_INTERVAL);
		double rep = event.time() + RandomUtils.getRandomTime(fit * I_POP_SIZE / MAX_POP_SIZE * REP_INTERVAL);
		//double dead = RandomUtils.getRandomTime(fit * D_INTERVAL);


		Event eventReproduction = new Event('r', rep, event.individual());
		

		fit = 1 - Math.log10(population.fitness(reproducedIndividual));
		double mut = event.time() + RandomUtils.getRandomTime(fit * MUT_INTERVAL);
		rep = event.time() + RandomUtils.getRandomTime(fit * I_POP_SIZE / MAX_POP_SIZE * REP_INTERVAL);
		double dead = event.time() + RandomUtils.getRandomTime(fit * D_INTERVAL);
		...
		if(debug){
			System.out.println("Individual reproduced fitness: " + fit);
	...
	private static void deathEvent() {
		...
		if (RandomUtils.getRandomEvent(1 - Math.pow(population.fitness(event.individual()), 2))) {
			...
			Event newEventDeath = new Event('d', event.time() + deathInterval(D_INTERVAL), event.individual());
			eventQueue.add(newEventDeath);
		}
	}
 ...
 /*
	*	Returns mutation interval
	...
	public static double mutationInterval(int MUT_INTERVAL) {
		double mut = RandomUtils.getRandomTime((1 - Math.log10(population.fitness(event.individual())) * MUT_INTERVAL));
		return mut;
	}

	/*
	*	Returns reproduction interval
	 */
	public static double reproductionInterval(int I_POP_SIZE, int REP_INTERVAL, int MAX_POP_SIZE) {
		double rep = RandomUtils.getRandomTime((1 - population.fitness(event.individual()) * I_POP_SIZE / MAX_POP_SIZE * REP_INTERVAL));
		System.out.println(event.individual());
		return rep;
	}

	/*
	*	Returns death interval
	 */
	public static double deathInterval(int D_INTERVAL) {
		double dead = RandomUtils.getRandomTime((1 - Math.log10(1 - population.fitness(event.individual())) * D_INTERVAL));
		return dead;
	}
	...
	* Prints the best individual's path by city names and total cost in euclidean distance.
	...
	public static void individuals(EventQueue eventQueue, Population population) {
		...
		double fit = (1 - (Math.log10(population.fitness(individual))));
		double mut = RandomUtils.getRandomTime(fit * MUT_INTERVAL);


		double rep = RandomUtils.getRandomTime(fit * I_POP_SIZE / MAX_POP_SIZE * REP_INTERVAL);
		double dead = RandomUtils.getRandomTime(1 - Math.log10(1-population.fitness(individual))) * D_INTERVAL;
		...
		/*
		System.out.println("Individual real fitness: " + population.fitness(individual));
		System.out.println("Individual 1- fitness: " + fit);
		System.out.println("fit * mutation interval before: " + fit * MUT_INTERVAL);
		System.out.println("Mutation: " + mut);
		System.out.println("fit * ipopsize/max * rep i: " + fit * I_POP_SIZE / MAX_POP_SIZE * REP_INTERVAL);
		System.out.println("Reproduction: " + rep);
		System.out.println("fit * death interval before: " + fit * D_INTERVAL);
